...
async function loadSentMessages() {
    ...
    try {
        const { data: { session } } = await supabase.auth.getSession();
        // Gọi API số 8 đã có sẵn trong index.js
        ...
        if (data.success && data.messages.length > 0) {
            ...
            data.messages.forEach(msg => {
                ...
                // Hiển thị người nhận (receiver_identifier)
                const receiverInfo = msg.receiver_identifier || 'Không rõ';
                const method = msg.receiver_method || '';

                html += `
                    <div class="message-item" style="border-left-color: #4dabf7;">
                        <div class="message-header">
                        ...
                        <div style="margin-top: 10px; font-size: 0.85rem; color: #aaa; font-style: italic;">
                            ${msg.is_anonymous ? '<i class="fas fa-user-secret"></i> Bạn đã gửi ẩn danh' : '<i class="fas fa-user"></i> Gửi công khai'}
                        </div>
                    </div>
                `;
            });
